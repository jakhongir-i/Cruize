...
const gulp = require("gulp"), // Сам сборщик Gulp
  scss = require("gulp-sass"), // Пакет компиляции SASS/SCSS
...
gulp.task("scss", () => {
  return gulp
    .src(`${dev}/scss/style.scss`) // В этом файле(-ах) хранятся основные стили, остальные следует импортировать в него
...
gulp.task("pug", () => {
  return gulp
    .src(`${dev}/pug/*.pug`) // файл(-ы) pug препроцессора
    .pipe(
      pug({
...
gulp.task("_pug", () => {
  return gulp
    ...
    .pipe(
      critical({
        // генерируем критический CSS для быстрой загрузки страниц
...
gulp.task("eslint", () => {
  return gulp
    .src([`${dev}/components/**/*.js`, `${dev}/assets/*.js`]) // все JS файлы
    .pipe(
      eslint({
        fix: true
      })
...
gulp.task(
  ...
  gulp.parallel("eslint", () => {
    return gulp
      .src(`${dev}/js/common.js`) // основной(-ые) файл(-ы) наших сценариев
      .pipe(
        plumber({
...
gulp.task("jsLibs", () => {
  return gulp
    .src(`${dev}/js/libs.js`) // файл, в который импортируются наши библиотеки
    .pipe(
      plumber({
        errorHandler: notify.onError({
          title: "JavaScript",
...
gulp.task("img", () => {
  return gulp
    .src(`${dev}/img/**/*`) // путь ко всем изображениям
    .pipe(
      cache(
        imagemin([
